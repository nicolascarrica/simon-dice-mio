...
function manejarInputUsuario(e) { // e viene event
    const $cuadro = e.target; // e.target para saber a que elemento le hicimos click
    resaltar($cuadro);
...
function obtenerCuadroAleatorio(){
    ...
    const indice = Math.floor(Math.random() * $cuadros.length); //math.random da numeros entre 0 y 1.
...
function perder() {
    bloquearInputUsuario();
    actualizarEstado('Perdiste!! Toca "Empezar" para volver a jugar', true);
}